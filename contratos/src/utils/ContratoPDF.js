...
//   "fechasalida_itinerario": document.getElementsByName("fechasalida_itinerario")[0].value,
//   "presentarse_itinerario": document.getElementsByName("presentarse_itinerario")[0].value,
//   "horasalida_itinerario": document.getElementsByName("horasalida_itinerario")[0].value,
...
//   "colonia_itinerario": document.getElementsByName("colonia_itinerario")[0].value,
//   "ciudad_itinerario": document.getElementsByName("ciudad_itinerario")[0].value,
...
//   "detalles_itinerario": document.getElementsByName("detalles_itinerario")[0].value,
//   "fecharegreso_itinerario": document.getElementsByName("fecharegreso_itinerario")[0].value,
//   "horaregreso_itinerario": document.getElementsByName("horaregreso_itinerario")[0].value,
...
function Contrato(doc, info, qr, nombre_contrato) {
  ...
  doc.setFontSize(11);
  doc.textWithLink("RECORRIENDO KILOMETROS SA DE CV", 160, 45, {
    url: "http://recorriendokilometros.com.mx/",
  });
  ...
  doc.text(
    info.fechasalida_itinerario ? info.fechasalida_itinerario : "",
    444,
    starty + 13
  );
  ...
  doc.setTextColor(color_valores);
  doc.text(
    info.presentarse_itinerario ? info.presentarse_itinerario : "",
    130,
    starty + 13
  );
  doc.line(15, starty + 17, 565, starty + 17);
  ...
  doc.setTextColor(color_valores);
  doc.text(
    info.horasalida_itinerario ? info.horasalida_itinerario : "",
    444,
    starty + 13
  );
  ...
  doc.setTextColor(29, 34, 78);
  doc.textWithLink(
    info.ubicacion_direccion_salida_itinerario,
    165,
    starty + 13,
    { url: info.ubicacion_direccion_salida_itinerario }
  );
  doc.setTextColor(color_azules);
  doc.line(15, starty + 17, 565, starty + 17);
  ...
  let splitpuntoreferencia = doc.splitTextToSize(
    info.referencias_itinerario,
    390
  );
  ...
  doc.setTextColor(color_valores);
  doc.text(
    info.horaregreso_itinerario ? info.horaregreso_itinerario : "",
    450,
    starty + 13
  );
  ...
  doc.setTextColor(color_azules);
  doc.text(30, starty + 5, "ITINERARIO");
  doc.setTextColor(color_valores);

  doc.roundedRect(30, starty + 8, 520, 98, 3, 3, "D");

  // var splitObservacion = ""
  var splitObservacion = doc.splitTextToSize(info.detalles_itinerario, 510);

  doc.text(40, starty+increment, splitObservacion);
  ...
  var splitObservacion = doc.splitTextToSize(
  ...
  try {
    ...
    doc.text(
      120,
      starty,
      info.total_pagos.replace("$", "").replace(",", "").trim()
    );
  }
  ...
  doc.setTextColor(color_azules);
  var derivado = doc.splitTextToSize(
    "Derivado del Servicio de Transporte Terrestre que solicite a la empresa RECORRIENDO KILOMETROS S.A. de C.V., y una vez que he leido el total de las condiciones que se adjuntan al presente contrato, estoy de acuerdo con el total de su contenido.",
    530
  );
  ...
  try {
    ...
    doc.text(500, starty + 55, info.nombre_contrato);
  ...
  doc.text(
    20,
    starty,
    "NOTA IMPORTANTE: A la firma del contrato deberá cubrirse el 20% del importe total, y el saldo una semana antes de la realización del viaje."
  );
...
function pieCharter(doc) {
  ...
  const salidas2 =
    "5:30 am Soriana Rio Nilo a un costado de Banamex (Rio Nilo y Patria)";
  starty += incremento;
  ...
  doc.setTextColor(7, 109, 150); // azul blue
  const leyenda1 =
    "TELEFONO DE EMERGENCIA EN LA SALIDA 333-808-6093 CON GUSTAVO JAUREGUI";
  starty += incremento;
  ...
  const leyenda3 =
    "Av. Fidel Velazquez #643-A, Col. Santa Elena Alcande., Cp.P. 44230, Guadalajara, Jal. ";
  starty += incremento;
